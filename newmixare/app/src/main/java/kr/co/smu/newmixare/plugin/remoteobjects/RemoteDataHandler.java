package kr.co.smu.newmixare.plugin.remoteobjects;
...
import kr.co.smu.newmixare.data.DataHandler;
import kr.co.smu.newmixare.data.convert.DataProcessor;
import kr.co.smu.newmixare.lib.marker.InitialMarkerData;
import kr.co.smu.newmixare.lib.marker.Marker;
import kr.co.smu.newmixare.lib.marker.draw.ParcelableProperty;
import kr.co.smu.newmixare.lib.marker.draw.PrimitiveProperty;
...
public class RemoteDataHandler extends DataHandler implements DataProcessor{
    ...
    private Marker fillExtraMarkerParcelableProperties(Marker marker, Map<String, ParcelableProperty> properties){
        Iterator<Map.Entry<String, ParcelableProperty>> it = properties.entrySet().iterator();
        while(it.hasNext()){
            Map.Entry<String, ParcelableProperty> pairs = (Map.Entry<String, ParcelableProperty>)it.next();
    ...
    private Marker fillExtraMarkerPrimitiveProperties(Marker marker, Map<String, PrimitiveProperty> properties){
        Iterator<Map.Entry<String, PrimitiveProperty>> it = properties.entrySet().iterator();
        while(it.hasNext()){
            Map.Entry<String, PrimitiveProperty> pairs = (Map.Entry<String, PrimitiveProperty>)it.next();
            marker.setExtras(pairs.getKey(), pairs.getValue());
        }
        return marker;
    }
